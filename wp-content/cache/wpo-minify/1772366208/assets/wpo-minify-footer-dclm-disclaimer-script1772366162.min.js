...
av_setCookie=function(e){e.preventDefault();var is_legal=jQuery(e.currentTarget).attr('rel');var cookieDuration=parseInt(dclm_ajax_var.disclaimer_cookie_duration,10);var cookieOptions={path:'/'};if(cookieDuration>0){cookieOptions.expires=new Date(new Date().getTime()+cookieDuration*3600*1000,)}
jQuery.cookie('is_legal',is_legal,cookieOptions);if(is_legal=='yes'){if(dclm_ajax_var.disclaimer_redirect_url!=''){if(dclm_ajax_var.disclaimer_redirect_stay_on_site=='on'){window.open(window.location.href,'_blank');window.location.replace(dclm_ajax_var.disclaimer_redirect_url)}else{window.open(dclm_ajax_var.disclaimer_redirect_url,'_blank')}}
